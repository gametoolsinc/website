<?php
include_once($_SERVER['DOCUMENT_ROOT'] . "/library/server/template/webpage/webpage.php");
include_once($_SERVER['DOCUMENT_ROOT'] . "/module/module.php");

class Dice extends Module {
    private string $width;

    function __construct(Webpage $webpage){
        $this->width = "100%";
        $webpage->addStylesheet("/module/dice/stylesheet.css");
        $webpage->addJavascript("/module/dice/javascript.js");
    }

    function setWidth(string $width){
        $this->width = $width;
    }

    function place(){
        echo "<div class='dice' onclick='rollDice()' style='width: {$this->width};'></div>";
    }
}



if (basename(__FILE__) == basename($_SERVER["SCRIPT_FILENAME"])) {
    $webpage = new Webpage();
    $webpage->startContent();
    $dice = new Dice($webpage);
    $dice->setWidth("100px");
    $dice->place();
    $webpage->endContent();
    $webpage->generateWebpage();
}