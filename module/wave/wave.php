...
class Waves extends Module
{
    ...
    function place()
    {
        ...
        for ($i = 0; $i < $this->amount_of_layers; $i++) {
            ...
            for ($j = 0; $j <= $this->amount_of_points; $j++) {
                if ($previousLayer == false) {
                    $offset = random_int(0, $this->height_layer);
                } else {
        ...
        for ($i = 0; $i < $this->amount_of_layers; $i++) {
            ...
            for ($j = 0; $j <= $this->amount_of_points; $j++) {
                $point = $layers[$i][$j];
                $dx2 = $point[0] - random_int($this->space_between_points / 3, $this->space_between_points / 3 * 2);
                $dy2 = $point[1] - random_int(-$this->height_layer / 4, $this->height_layer / 4);
                if ($j == 0) {
                    $d .= "M $point[0] $point[1]";
                } else if ($j == 1) {
